...
class Logformat_mIRC (Logformat):
    ...
    def _now2(self, timestamp):
        return time.strftime("%a %b %d of %Y at %H:%M:%S", time.localtime(timestamp))
...
class Logutil:
    ...
    def get_users(self):
        query="""SELECT quasseluser.username FROM public.quasseluser"""
        cur = self.cursor()
        cur.execute(query)
    ...
    def get_networks(self, username):
        ...
        FROM public.network, public.quasseluser
        WHERE quasseluser.username=(%s) AND
        network.userid = quasseluser.userid
        """
        cur = self.cursor()
    ...
    def get_buffers(self, username, network):
        query = """
        SELECT buffer.buffername
        FROM public.buffer, public.quasseluser, public.network
        WHERE quasseluser.username=(%s) AND buffer.networkid = network.networkid
        AND network.networkname=(%s) AND buffer.userid = quasseluser.userid
        """
        nwquery =  """
        SELECT network.networkname,
        FROM public.network, public.quasseluser
        WHERE username=(%s) AND network.userid = quasseluser.userid
        """
        cur = self.cursor()
        if network is None:
            result = {}
    ...
    def getlog(self, username, network, buffer, outstream,iter=0, fmt=Logformat_mIRC(buffer), counter=None, log="", limit='all', time=False):
        """gets a logfile
        ...
        cur = self.cursor()
        query = """
        SELECT backlog.type, backlog.flags, EXTRACT(EPOCH FROM backlog.time), sender.sender, backlog.message
        FROM public.backlog, public.sender, public.buffer, public.network, public.quasseluser
        WHERE username=(%s) And backlog.senderid = sender.senderid
        AND networkname=(%s) AND backlog.bufferid = buffer.bufferid
        AND buffername=(%s) AND buffer.networkid = network.networkid
        AND buffer.userid = quasseluser.userid AND time > (%s)
        ORDER BY backlog.messageid ASC
        LIMIT (%s)
        OFFSET (%s)
        """
        cur.execute(query, (username, network, buffer,time,limit, iter*limit1))
        rowcounts=cur.rowcount
        if not counter is None and rowcounts < limit:
            counter.set_max(rowcounts-1)
            counter.start()
        for x in range(1, rowcounts):
            if not counter is None and rowcounts < limit:
                counter.next()
            row = cur.fetchone()
        ...
        if rowcounts==limit:
            log.getlog(username, network, buffer, outstream, iter+1, log=log, counter=counter, limit=limit, time=time)
